...
//   if (ret == -1)
//     return err(errno).reason("Failed to eventfd.").build();

//   auto res = Fd::from_raw_fd(ret);
//   if (!res)
...
//   if (ret == -1)
//     return err(errno).reason("eventfd for io_uring_event_fd
//     failed.").build();
...
//   io_uring_event_fd}}; ret = epoll_ctl(epoll_fd, EPOLL_CTL_ADD,
//   io_uring_event_fd, &ev); if (ret == -1)
//     return err(errno)
...
//   if (ring_)
//     return err(Errc::already_opened).reason("io_uring already
//     opened.").build();
...
//   struct epoll_event ev{.events = EPOLLIN | EPOLLET, .data = {.fd =
//   *event_fd}}; if (int ret = epoll_ctl(*epoll_fd_, EPOLL_CTL_ADD, *event_fd,
//   &ev); ret == -1)
//     return err(errno)
